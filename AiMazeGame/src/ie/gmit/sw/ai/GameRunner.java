...
public class GameRunner implements KeyListener {
	...
	public void keyPressed(KeyEvent e) {
		...
		if (e.getKeyCode() == KeyEvent.VK_RIGHT && game.getPlayer().getColPos() < MAZE_DIMENSION - 1) {
			if (isValidMove(game.getPlayer().getRowPos(), game.getPlayer().getColPos() + 1)
			...
			if (isValidMove(game.getPlayer().getRowPos() - 1, game.getPlayer().getColPos())
	...
	private boolean isValidMove(int r, int c) {
		...
		case 'W':
			...
			return true;
		case '?':
			Traversator traverse = algorithm(0); // Try to make it do A*. Change algorithm(i) for other searches, i = 1, i = 2 etc...
			traverse.traverse(game.getMaze(),
					game.getMaze()[game.getPlayer().getRowPos()][game.getPlayer().getColPos()]);
		...
		case 'M':
			...
			return true;
		case 'A':
			// Health pick up, adds 50 armor to players character
		...
		case 'G':
			...
			JOptionPane.showMessageDialog(null, "You have got to the exit, game won.", "Info: " + "Success!",
	...
	private Traversator algorithm(int randNum) {
		...
		switch (randNum) {
		case 0:
			System.out.println("[INFO] Calling A* Search.");
